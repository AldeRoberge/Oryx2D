...
public class GetCharListTask extends BaseTask {
	...
	public Object makeRequestData() {
		/*
		 * var loc1:Object = {}; loc1.game_net_user_id =
		 * this.account.gameNetworkUserId(); loc1.game_net = this.account.gameNetwork();
		 * loc1.play_platform = this.account.playPlatform(); loc1.do_login =
		 * Parameters.sendLogin; MoreObjectUtil.addToObject(loc1,
		 * this.account.getCredentials()); return loc1;
		 */
		return null;
	}
	...
	private void onListComplete(String param1) {
		...
		if (loc2.hasOwnProperty("MigrateStatus")) {
			...
			/*
			 * loc4 = new MigrationDialog(this.account, loc3); this.fromMigration = true;
			 * loc4.done.addOnce(new SignalConsumer<>(this::sendRequest));
			 * loc4.cancel.addOnce(new
			 * SignalConsumer<>(this::clearAccountAndReloadCharacters));
			 */

			System.out.println("Migration needed!");
	...
	private void waitForASecondThenRetryRequest() {
		...
		// this.retryTimer.addEventListener(TimerEvent.TIMER_COMPLETE, new
		// EventConsumer<>(this::onRetryTimer));
		this.retryTimer.start();
	}

	private void stopRetryTimer() {
		this.retryTimer.stop();
		// this.retryTimer.removeEventListener(TimerEvent.TIMER_COMPLETE, new
		// EventConsumer<>(this::onRetryTimer));
