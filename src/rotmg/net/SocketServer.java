...
/**
 * This class is a very loose implementation of WildShadow's SocketServer, it is
 * more closely related to The Force 2477's RealmClient
 */
public class SocketServer {
			...
			public void run() {
				...
				try {
					while ((sock != null) && sock.isConnected() && SocketServer.this.read && !sock.isClosed()) {
						Thread.sleep(20);
						int bytesRead = SocketServer.this.inputStream.read(SocketServer.this.buffer,
								SocketServer.this.bufferIndex,
								SocketServer.this.buffer.length - SocketServer.this.bufferIndex);
						if (bytesRead == -1) {
							...
							while (SocketServer.this.bufferIndex >= 5) {
								int packetLength = ((ByteBuffer) ByteBuffer.allocate(4).put(SocketServer.this.buffer[0])
										.put(SocketServer.this.buffer[1]).put(SocketServer.this.buffer[2])
										.put(SocketServer.this.buffer[3]).rewind()).getInt();
								...
								if (SocketServer.this.bufferIndex > packetLength) {
									System.arraycopy(SocketServer.this.buffer, packetLength, SocketServer.this.buffer,
											0, SocketServer.this.bufferIndex - packetLength);
								}
								...
								if (m == null) {
									...
									// System.out.println("Received a " + m.getClass().getSimpleName() + "
									// packet.");
						...
						int lastPacket = (int) ((System.currentTimeMillis()
								- SocketServer.this.lastTimePacketReceived));

						/*
						 * if (lastPacket > 2000 && !packetQueue.isEmpty()) { reconnect(
	...
	private void startThreadedWriter() {
		...
		new Thread("threadedWriter") {
			@Override
			public void run() {
				while ((SocketServer.this.socket != null) && SocketServer.this.socket.isConnected()
						&& SocketServer.this.write) {
